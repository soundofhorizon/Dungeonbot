...
class Mmorpg(commands.Cog):
    ...
    async def mr(self, ctx):
        try:
            ...
            uuid_list = [[str(item[0]), int(item[1]), int(item[2]), int(item[3]), int(item[4]), int(item[5]), int(item[6]), int(item[7])] for item in mmo_data]
            player_data = ""
            for i,j,k,l,m,n,o,p in uuid_list:
                ...
                if i == str(ctx.author.id):
                    ...
                    if response.content == "L":
                        ...
                        await show_embed.edit(embed=self.bot.edit_embed(show_embed, "ラスボスPhase", f"貴方はラスボスに遭遇した。ラスボスのHPは残り{colon_formatted_number(m)}。\n攻撃を行うなら**A**と入力せよ。"))
                    ...
                    # 子分関係
                        ...
                        await ctx.channel.purge(limit=1)
                        await show_embed.edit(embed=self.bot.edit_embed(show_embed, "子分Phase", f"貴方はラスボスの子分の{weak_boss_name[o]}に遭遇した。{weak_boss_name[o]}のHPは残り{colon_formatted_number(n)}。\n攻撃を行うなら**A**と入力せよ。"))
                        response = await self.bot.wait_for('message', check=check2)
                        if response.content == "A":
                            ...
                            async def EnemyAttack(Type, show_embed, temp_player_data):
                                ...
                                if Type == 0:
                                    ...
                                    await show_embed.edit(embed=self.bot.edit_embed(show_embed, "Enemy Turn", description))
                                elif Type == 1:
                                    ...
                                    if ishit > 80:
                                        ...
                                        await show_embed.edit(embed=self.bot.edit_embed(show_embed, "Enemy Attack!", description))
                    ...
                    # SHOP関係
                        ...
                        await ctx.channel.purge(limit=1)
                        await show_embed.edit(embed=self.bot.edit_embed(show_embed, "SHOP", f"ここは、貴方が持っている全てのcoinsを生贄にし、random.randint(0,yourcoins)×random.random()で決定されたstr分加算されるSHOPだよ…\n\n …やってくかい？\n\nstrを購入するならP, 中止するならCを入力せよ。\n\n 貴方の所持コイン: {colon_formatted_number(l)}coins"))
                        response = await self.bot.wait_for('message', check=check3)
                        if response.content == "P":
                            await ctx.channel.purge(limit=1)
                            await show_embed.edit(embed=self.bot.edit_embed(show_embed, "Purchase Processing...", "ひひひ…"))
                            await asyncio.sleep(3)
                            div_str = random.randint(1,4)
                            result = int(random.randint(0,int(l/div_str))*random.random())
                            await show_embed.edit(embed=self.bot.edit_embed(show_embed, "Purchase Complete!", f"{ctx.author.display_name}のstrが{colon_formatted_number(result)}上昇した！"))
                            temp_player_data = f"{i} {j} {k+result} {0} {m} {n} {o} {p}"
                        elif response.content == "C":
                            await show_embed.edit(embed=self.bot.edit_embed(show_embed, "Purchase Cancelled", "ちっ…冷やかしか…"))
                            temp_player_data = f"{i} {j} {k} {l} {m} {n} {o} {p}"
            ...
            await show_embed.edit(embed=self.bot.edit_embed(show_embed, "Error", f"Request Error. 少々お待ちいただき再度お試しください。\n\n{e}"))
